...
export class ExampleTsStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const saveHelloFunction = new lambda.Function(this, "SaveHelloFunction", {
      ...
      code: lambda.Code.fromAsset(path.resolve(__dirname, "../resources"), { exclude: ['**', '!handler.js'] }),
      environment: {
    ...
    helloApi.root.resourceForPath("hello").addMethod("POST", new apigw.LambdaIntegration(saveHelloFunction));
    helloApi.root.resourceForPath("hello").addMethod("GET", new apigw.LambdaIntegration(getHelloFunction));
